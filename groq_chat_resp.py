#!/usr/bin/env python3
"""
Groq Chat Responses API Client

This module provides a client for the Groq Responses API, offering identical functionality
to the Chat Completions API but with enhanced conversation management features.
"""
...
class GroqChatResp:
    ...
    def _extract_function_info(self, func: t.Callable) -> dict:
        """Extract function information for Groq Responses API tool definition."""
                ...
                if len(tool_calls) > 1:
                    ...
                    for tool_call in tool_calls:
                        ...
                        if function_name in tool_results:
                            # Create the assistant message with tool calls (Responses API format)
                            assistant_message = {
                                "role": "assistant",
    ...
    def _invoke_groq_responses_api(self, query: t.Optional[str] = None, json_schema: t.Optional[dict | t.Any] = None, 
        ...
        # Add the assistant response with tool calls (if any)
        ...
        if tool_execution_results:
            for tool_call in tool_calls:
                function_name = tool_call.get("function", {}).get("name", "")
                if function_name in tool_execution_results:
                    tool_message = {
                        "role": "tool",
                        "content": str(tool_execution_results[function_name]),
                        "tool_call_id": tool_call.get("id")
                    }
...
if __name__ == "__main__":
    """Example usage with Groq Responses API."""
    ...
    try:
        ...
        print("=== Groq Chat Responses Configuration ===")
